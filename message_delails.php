 ...
 <!-- ************* Navbar Start *************-->
  ...
  <!-- ********************* Side Bar Add Start*******************--> 
   ...
   <ul class="nav nav-pills nav-stacked" style=" margin-top: 70px;">
      ...
      <li class="active" ><a  id=""  href="javascript:void(0)" style="font-size: 15px;padding: 14px; text-align: center; color: #fff" > <span class="glyphicon glyphicon-eye-open"  style="margin-right: 10px" aria-hidden="true"></span>View Website</a></li>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
	  ...
	  <form action="Content_update/content_update_messege_details.php" method="get">
...
$conn=mysqli_connect("localhost","root","","literatute_club"); 
				...
				$eql="SELECT * FROM messeges where id=$nid";
              $result = $conn->query($eql);
		  while($row = $result->fetch_assoc()) 
		  {
			
		     echo'<form id="" action="messages_update.php" method="post" enctype="multipart/form-data">';
			
	         echo'<div class="form-group">
				    <label for="">Person Name:</label>
				    <input type="text" class="form-control" name="pname"id="pname" value="'.$row["personname"].'" required>
				  </div>

				  <div class="form-group">
				    <label for="">Designation:</label>
                       <input type="text" class="form-control" name="des" id="des" required value="'.$row["designation"].'" required>				     
				  </div>
				  <div class="form-group">
				  ...
				  <div class="form-group">
				    ...
				    <textarea class="form-control" rows="6" name="message" placeholder="" value="'.$row["messeges"].'">'.$row["messeges"].'</textarea>
...
var h = document.getElementById("pname").value;
