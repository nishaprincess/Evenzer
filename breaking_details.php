 ...
 <!-- ************* Navbar Start *************-->
  ...
  <div class="modal-dialog">
    <div class="modal-content">
	  ...
	  <form action="Content_update/content_update_breaking_details.php" method="get">
...
$conn=mysqli_connect("localhost","root","","literatute_club"); 
				...
				$eql="SELECT * FROM breaking where id=$nid";
              $result = $conn->query($eql);
		  while($row = $result->fetch_assoc()) 
		  {
			
		     echo'<form id="" action="breaking_update.php" method="post" >';
			
	         echo'<div class="form-group">
				    <label for="">Header</label>
				    <input type="text" class="form-control" name="head" id="head" value="'.$row['head'].'" required>
				  </div>

				  <div class="form-group">
				    <label for="">Description</label>
				    <textarea class="form-control" rows="6" name="bdesc" id="bdesc" required value="'.$row['bdesc'].'">'.$row['bdesc'].'</textarea>
				     
				  </div>
				  <div class="form-group">
				    <label for="">Linkname:</label>
				   <input type="text" class="form-control" name="lname" value="'.$row['linkname'].'">
...
var d = document.getElementById("bdesc").value;
  if(h && d) {   
