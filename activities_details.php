 ...
 <!-- ************* Navbar Start *************-->
  ...
  <div class="modal-dialog">
    <div class="modal-content">
	  ...
	  <form action="Content_update/content_update_activities_details.php" method="get">
...
$conn=mysqli_connect("localhost","root","","literatute_club"); 
				...
				$eql="SELECT * FROM activities where id=$nid";
              $result = $conn->query($eql);
		  while($row = $result->fetch_assoc()) 
		  {
			
		     echo'<form id="" action="activities_update.php" method="post" >';
			
	         echo'<div class="form-group">
				    <label for="">Name:</label>
				    <input type="text" class="form-control" id="aname"name="aname" value="'.$row['aname'].'" required>
				  </div>

				  <div class="form-group">
				    <label for="">Details:</label>
				    <textarea class="form-control" rows="6" name="adesc" id="adesc"  value="'.$row['adesc'].'" required>'.$row['adesc'].'</textarea>
...
var h = document.getElementById("aname").value;
var d = document.getElementById("adesc").value;
  if(h && d) {   
